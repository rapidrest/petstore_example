...
import { getUserCRUDRoute, getTrustedRoles } from "@/src/routes/userRouteHelper";
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
    ...
    try {
        ...
        const modelRoute = await getUserCRUDRoute();
        await modelRoute.validate(obj, { user: auth.user });
        ...
        const result = await modelRoute.update(id, obj, request as any, auth.user);
        return NextResponse.json(result);
    } catch (err: any) {
        return apiError(err, 400);
    }
